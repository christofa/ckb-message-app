...
export async function fetchMessages(): Promise<{ message: string; txHash: string }[]> {
  ...
  // findCellsByLock = "find all boxes locked with MY key"
  ...
  for await (const cell of collector) {
    ...
    try {
      ...
      // (some cells might have non-text data we can't read)
